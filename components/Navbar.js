'use client'

import React from 'react'
import Link from 'next/link'
import { useSession, signOut } from 'next-auth/react'
import { useRouter } from 'next/navigation'

const Navbar = () => {
  const { data: session, status } = useSession();
  const router= useRouter();

    if (status === 'loading') return null;
  

  return (
<div className="bg-blue-700 flex justify-between items-center p-6 px-6 sticky top-0">     
      <ul className="text-white flex gap-3 justify-center">
        <li><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className="injected-svg" data-src="https://cdn.hugeicons.com/icons/train-01-solid-rounded.svg?v=2.0" role="img" color="#ffffff">
<path fillRule="evenodd" clipRule="evenodd" d="M7.55471 18.0429C8.01424 18.3493 8.13842 18.9702 7.83207 19.4297L5.83206 22.4297C5.52571 22.8892 4.90484 23.0134 4.44531 22.707C3.98579 22.4007 3.86161 21.7798 4.16796 21.3203L6.16796 18.3203C6.47432 17.8608 7.09519 17.7366 7.55471 18.0429ZM16.4453 18.0429C16.9048 17.7366 17.5257 17.8608 17.8321 18.3203L19.8321 21.3203C20.1384 21.7798 20.0142 22.4007 19.5547 22.707C19.0952 23.0134 18.4743 22.8892 18.168 22.4297L16.168 19.4297C15.8616 18.9702 15.9858 18.3493 16.4453 18.0429Z" fill="#ffffff"></path>
<path fillRule="evenodd" clipRule="evenodd" d="M11.9289 1.125H12.0711H12.0712C13.8439 1.12498 15.2729 1.12496 16.3967 1.27628C17.5635 1.43339 18.546 1.7695 19.3262 2.55093C20.1065 3.33237 20.4421 4.31627 20.599 5.48483C20.75 6.61038 20.75 8.04147 20.75 9.81695V10.9331C20.75 12.5813 20.75 13.9328 20.6292 15.0185C20.6221 15.0817 20.5635 15.125 20.5 15.125H18.75C18.3358 15.125 18 15.4608 18 15.875C18 16.2892 18.3358 16.625 18.75 16.625H19.4483C19.8434 16.625 20.041 16.625 20.1276 16.775C20.2142 16.9249 20.1243 17.0807 19.9445 17.3923C19.7777 17.6813 19.5745 17.9504 19.3262 18.1991C18.546 18.9805 17.5635 19.3166 16.3967 19.4737C15.2729 19.625 13.8439 19.625 12.0712 19.625H11.9289C10.1561 19.625 8.72713 19.625 7.60329 19.4737C6.43648 19.3166 5.45405 18.9805 4.67379 18.1991C4.42553 17.9504 4.22229 17.6813 4.05549 17.3923C3.87572 17.0807 3.78583 16.9249 3.87244 16.775C3.95905 16.625 4.1566 16.625 4.55171 16.625H5.25C5.66421 16.625 6 16.2892 6 15.875C6 15.4608 5.66421 15.125 5.25 15.125H3.5C3.43646 15.125 3.37788 15.0817 3.37085 15.0185C3.24996 13.9328 3.24998 12.5814 3.25 10.9331V10.9331V9.8169V9.81688C3.24998 8.04144 3.24996 6.61036 3.40105 5.48483C3.55792 4.31627 3.89353 3.33237 4.67379 2.55093C5.45405 1.7695 6.43648 1.43339 7.60329 1.27628C8.72712 1.12496 10.1561 1.12498 11.9288 1.125H11.9289ZM5.8698 5.625H18.13C18.3819 5.625 18.5078 5.625 18.5938 5.70395C18.6799 5.7829 18.6903 5.9041 18.7112 6.14649C18.7922 7.08623 18.8054 8.33226 18.8054 9.88816L18.8053 10.8959C18.8053 11.1297 18.8053 11.2466 18.7369 11.3297C18.6685 11.4127 18.5552 11.4349 18.3286 11.4793C18.0711 11.5297 17.785 11.5826 17.4743 11.6356C16.0344 11.881 14.0786 12.1249 12.0003 12.1249C9.92209 12.1249 7.96634 11.881 6.52637 11.6356C6.2153 11.5825 5.92889 11.5295 5.67117 11.4791C5.44456 11.4347 5.33125 11.4125 5.26287 11.3295C5.19449 11.2464 5.19448 11.1295 5.19445 10.8957L5.19434 9.88816C5.19434 8.33226 5.20757 7.08623 5.28856 6.14649C5.30945 5.9041 5.3199 5.7829 5.40594 5.70395C5.49199 5.625 5.61793 5.625 5.8698 5.625Z" fill="#ffffff"></path>
</svg></li>
        <li><Link href="/">Home</Link></li>
        <li><Link href="/about">About</Link></li>
        <li><Link href="/contact">Contact</Link></li>
      </ul>
        {session ? (
      <div className="text-white flex gap-3 justify-center">
        {console.log(session.user)}
        {session.user.role=='admin' ?(
          <button onClick={()=>{router.push("/admin")}} className="cursor-pointer rounded-lg p-2 bg-white text-black hover:outline-2 hover:bg-gray-100">Admin</button>
        ) :(        
          <button onClick={()=>{router.push(`/profile/${session.user.username}`)}} className="cursor-pointer rounded-lg p-2 bg-white text-black hover:outline-2 hover:bg-gray-100">Profile</button>
)}
        <button onClick={()=>{router.push("/dashboard")}} className="cursor-pointer rounded-lg p-2 bg-white text-black hover:outline-2 hover:bg-gray-100">Dashboard</button>
          <button onClick={()=>signOut({callbackUrl:'/'})} className="cursor-pointer rounded-lg p-2 bg-red-400 text-white hover:outline-2">Logout</button>
      </div>
        ) : (
          <button onClick={()=>{router.push("/login")}} className="cursor-pointer rounded-lg p-2 bg-green-500 text-white hover:outline-2 hover:outline-black">Login</button>
        )}
    </div> 
  )
}

export default Navbar
